    ...
    public class BooksViewModelProfile : Profile
    {
        public BooksViewModelProfile()
        {
            CreateMap<Book, BookViewModel>()
                ...
                .AfterMap((b, bdto) => { bdto.Authors = Mapper.Map<ICollection<AuthorViewModel>>(b.BookAuthors.Select(a => a.Author).ToList()); })
                .AfterMap((b, bdto) => { bdto.Genres = Mapper.Map<ICollection<GenreViewModel>>(b.BookGenres.Select(a => a.Genre).ToList()); })
                .AfterMap((b, bdto) => { bdto.Tags = Mapper.Map<ICollection<TagViewModel>>(b.BookTags.Select(a => a.Tag).ToList()); });
